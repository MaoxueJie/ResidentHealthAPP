...
public class CordovaHttpGet extends CordovaHttp implements Runnable {
    ...
    public void run() {
        try {
            boolean base64 = this.getHeaders().get("Accept")==null?false:this.getHeaders().get("Accept").equalsIgnoreCase("base64");
            ...
            String body = "";
            if (base64)
                body = HttpRequest.Base64.encodeBytes(request.bytes());
            else
